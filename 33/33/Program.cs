...
namespace LibraryApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                connection.Open();
                adapter.SelectCommand = new SqlCommand("SELECT * FROM Читатели", connection);
                adapter.Fill(dataSet, "Читатели");
                adapter.SelectCommand = new SqlCommand("SELECT * FROM Книги", connection);
        ...
        static void AddMest(SqlConnection connection, int bookId, int readerId)
        {
            ...
            adapter.InsertCommand.ExecuteNonQuery();
            dataSet.Clear();
            adapter.Fill(dataSet, "Местонахождения");
        }
